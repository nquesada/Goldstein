...
Sea $u$ una funci\'on de $r^2,p^2,\textbf{r} \cdot \textbf{p}$. Para mostrar que $[u,\textbf{L}]=0$ es suficiente ver que $[r^2,\textbf{L}]=0$, $[p^2,\textbf{L}]=0$ y $[\textbf{r} \cdot \textbf{p},\textbf{L}]=0$, ya que:
\eq{
...
Pero $[x_i,x_j]=[p_i,p_j]=0$ y $[x_i,p_j]=-[p_j,x_i]=\delta_{ij}$. As\'i entonces:
\eq{
...
Finalmente:
\eq{
[x_l p_l, \epsilon_{i j k} x_i p_j]=&\epsilon_{i j k} ([x_l p_l,x_i]p_j+x_i [x_l p_l,p_j]) \nonumber\\
=&-\epsilon_{i j k}(p_j([x_i,x_l]p_l+x_l [x_i,p_l])+x_i ([p_j,x_l]p_l+x_l[p_j,p_l])) \nonumber \\
=&-\epsilon_{i j k} p_j x_i+\epsilon_{i j k} x_i p_j =0.
}

Si $\textbf{F}=u \textbf{r}+v\textbf{p}+w \textbf{r} \times \textbf{p}$, donde $u,v,w$ son funciones del mismo tipo que las del literal anterior, se pide mostrar que $[F_i,L_j]=\epsilon_{i j k} F_k$. Para ver lo anterior calculemos por separado las siguientes expresiones:
\eq{
[u x_i, L_j]=u [x_i,L_j],\\
[v p_i, L_j]=v [p_i,L_j],\\
[w L_i, L_j]=w [L_i,L_j],
...
Para este problema tenemos que los momentos can\'onicos est\'an dados por:
\eq{
p_k=m \dot x_k +\frac{q}{2} \epsilon_{ijk} B_i x_j.
}
De estos se obtiene f\'acilmente que:
\eq{
...
Con estas se encuentran f\'acilmente los $\dot p_a$:
\eq{
...
Para probar que esto es cierto para la $n$-\'esima derivada se usa inducci\'on:\\
Se prob\'o para $n=1$, se cumple \emph{i.e.}: $\frac{d}{dt}\left( -\frac{\partial F}{\partial t} \right)=0$.\\
Ahora supongamos que se cumple para $n$, es decir: $\frac{d}{dt}\left( \frac{\partial^n F}{\partial t^n} \right)=[\frac{\partial^n F}{\partial t^n},H]+\frac{\partial^{n+1} F}{\partial t^{n+1}}=0$, pero como por hip\'otesis $\frac{\partial^n F}{\partial t^n}$ es constante de movimiento, entonces $[\frac{\partial^n F}{\partial t^n},H]$ tambi\'en lo es y por lo tanto $\frac{\partial^{n+1} F}{\partial t^{n+1}}=-[\frac{\partial^n F}{\partial t^n},H]$ tambi\'en es constante de movimiento.
\\
\\
Finalmente, tomando $F=x-\frac{pt}{m}$ y $H=\frac{p^2}{2m}$ se verifica f\'acilmente que:
\eq{
...
Como sabemos las soluciones expl\'icitas del problema, podemos ver que es exactamente $u$. Si $q=A \sin (\omega (t-t_0))$ entonces $p=m \omega A \cos( \omega (t-t_0))$ y la cantidad dentro del logaritmo es: $m \omega A e^{i (\omega (t-t_0))}$. De acuerdo a lo anterior:
\[
u=\ln m \omega A+i (\omega (t -t_0))-i \omega t=-i\omega t_0+\ln \sqrt{2 E m},
\]
donde se ha usado que $E=\frac{1}{2} m \omega^2 A^2$. $u$ no es m\'as que el logaritmo de una funci\'on de la energ\'ia m\'as $i$ veces la fase inicial del oscilador.

A partir de este c\'alculo tambi\'en podemos obtener que:
\eq{
[u,H] = [-i \omega t_0 + \ln \sqrt{2 E m}, H] = [-i \omega t_0, H] + [\ln \sqrt{2 E m}, H].
}
El segundo t\'ermino es cero y as\'i obtenemos $[-i \omega t_0, H] = i \omega$, o de manera m\'as sencilla encontramos que formalmente $[H,t_0]=1$.